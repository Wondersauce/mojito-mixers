...
var countryRegionData = require('country-region-data/dist/data-umd');
var index = require('../../../node_modules/country-codes-list/index.js');
var formatUtils = require('../../utils/formatUtils.js');
var errors_constants = require('../errors/errors.constants.js');

function _interopDefaultLegacy (e) { return e && typeof e === 'object' && 'default' in e ? e : { 'default': e }; }

var countryRegionData__default = /*#__PURE__*/_interopDefaultLegacy(countryRegionData);

const countryPrefixes = index["default"].customList("countryCode", "{countryCallingCode}");
...
function transformRawSavedPaymentMethods(rawSavedPaymentMethods = []) {
    return rawSavedPaymentMethods.map((_a) => {
        ...
        const { countryName = "", countryShortCode: countryCode = "", regions = [], } = countryRegionData__default["default"].find(({ countryShortCode }) => countryShortCode === billingDetails.country) || {};
...
function billingInfoToBillingDetails(billingInfo, paymentType = "CreditCard") {
    ...
    if (paymentType !== "Crypto") {
        billingDetails.name = formatUtils.fullTrim(billingInfo.fullName);
    }
...
function parseCircleError(error) {
    ...
    if (errors_constants.BUILT_IN_ERRORS.includes(name) || !message)
        return undefined;
            ...
            if (isCircleFieldErrorArray(parsedCircleErrors)) {
                parsedCircleErrors.forEach(({ location, message: circleErrorMessage }) => {
                    ...
                    sectionFieldErrors[inputName] = [
                        sectionFieldErrors[inputName],
                        formatUtils.formatSentence(circleErrorMessage.replace(searchRegExp, inputLabel).replace(" (was )", "")),
                    ].filter(Boolean).join(" / ");
...
exports.EMPTY_ADDRESS_ID = EMPTY_ADDRESS_ID;
exports.billingInfoToBillingDetails = billingInfoToBillingDetails;
