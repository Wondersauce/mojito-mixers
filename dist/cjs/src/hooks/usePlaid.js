...
var index_esm = require('../../node_modules/react-plaid-link/dist/index.esm.js');
...
let INITIAL_PLAID_OAUTH_FLOW_STATE = CheckoutOverlay_utils.getCheckoutModalState({ noClear: true });
function continuePlaidOAuthFlow() {
    return INITIAL_PLAID_OAUTH_FLOW_STATE.continueFlow &&
        ...
        CheckoutOverlay_utils.isCheckoutModalInfoPlaid(INITIAL_PLAID_OAUTH_FLOW_STATE);
}
function usePlaid(options) {
    ...
    const plaidOAuthFlowStateRef = React.useRef(INITIAL_PLAID_OAUTH_FLOW_STATE);
    ...
    const onSuccess = React.useCallback((publicToken, metadata) => {
        ...
        plaidOAuthFlowStateRef.current = INITIAL_PLAID_OAUTH_FLOW_STATE = CheckoutOverlay_constants.FALLBACK_MODAL_STATE_COMMON;
        if (!onSubmit)
            return;
    ...
    const { open: plaidLinkOpen, ready: plaidLinkReady, error: plaidLinkError, } = index_esm.usePlaidLink(config);
    React.useEffect(() => {
    ...
    const openLink = React.useCallback(() => {
        if (!plaidLinkReady || isPreparePaymentMethodLoading || !orgID || !invoiceID || !invoiceCountdownStart || !selectedBillingInfo || !linkToken)
            return;
        if (url_utils.isLocalhostOrStaging())
