...
function ClickAwayListener(props) {
  ...
  const handleRef = useForkRef["default"]( // @ts-expect-error TODO upstream fix
  ...
  const handleClickAway = useEventCallback["default"](event => {
    // Given developers can stop the propagation of the synthetic event,
    // we can only be confident with a positive value.
    const insideReactTree = syntheticEventRef.current;
    syntheticEventRef.current = false;
    const doc = ownerDocument["default"](nodeRef.current); // 1. IE11 support, which trigger the handleClickAway even after the unbind
  ...
  React__namespace.useEffect(() => {
    if (touchEvent !== false) {
      ...
      const doc = ownerDocument["default"](nodeRef.current);
...
if (process.env.NODE_ENV !== 'production') {
  ...
  ClickAwayListener['propTypes' + ''] = exactProp["default"](ClickAwayListener.propTypes);
}
